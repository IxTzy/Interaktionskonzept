...
def compareNupdate(message, data, location, List, allEntityList):
    ...
    if topic == "ambulances":
        ...
        update(topic, entity, data)
        decider(userReason=data[2], userLocation=location, FirefighterList=allEntityList[0],
                AmbulanceList=allEntityList[1], PoliceList=allEntityList[2], HospitalList=allEntityList[3])
...
def update(topic, entity, data):
    if topic == "hospitals":
        try:
            entity.updateValues(
                hospitalName=data[0], doctors=data[2], freeRooms=data[4], specialists=data[5], id=data[3])
            pass
        except Exception as e:
            print(e)
            raise
        ...
        try:
            entity.updateValues(
                driverName=data[0], location=data[1], reasons=data[2])
            pass
        except Exception as e:
            print(e)
            raise
...
def decider(userReason=None, userLocation=None, FirefighterList=None, AmbulanceList=None, PoliceList=None, HospitalList=None):
...
def alarm(alarmList, userReason, userLocation):
    ...
    for data in alarmData:
        try:
            ...
            message = "User reason: " + userReason + ". User Location: " + \
